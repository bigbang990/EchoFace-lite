...
with observability_tab:
    ...
    if refresh_metrics or auto_metrics:
        ...
        if "error" in metrics_data:
            ...
            m4.metric("Avg FPS (wall-clock)", f"{averages.get('avg_fps_wall_clock', 0):.3f}", help="End-to-end throughput: total frames / total job wall-clock time (includes I/O, DB, preview writes)")
            m5.metric("AI Pipeline FPS", f"{averages.get('average_processing_fps', 0):.1f}", help="Throughput inside process_frame() only — excludes video decode, disk I/O, DB writes")
